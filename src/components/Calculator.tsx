...
const Calculator = () => {
  ...
  const [materials, setMaterials] = useState<{ product: Product; quantity: number }[]>([]);
  const [services, setServices] = useState<Service[]>([]);

  // Добавление материала
  const addMaterial = () => {
    if (selectedProduct && productQuantity > 0) {
      setMaterials([...materials, { product: selectedProduct, quantity: productQuantity }]);
  ...
  const addService = () => {
    if (serviceName && servicePrice > 0 && serviceQuantity > 0) {
      setServices([...services, { name: serviceName, price: servicePrice, quantity: serviceQuantity }]);
  ...
  const calculateTotal = () => {
    const materialsTotal = materials.reduce((sum, item) => sum + item.product.price * item.quantity, 0);
  ...
  return (
    <div className="container mt-4">
        ...
        <tbody>
          {materials.map((item, index) => (
            <tr key={`material-${index}`}>
              ...
              <td>
                <button className="btn btn-danger btn-sm" onClick={() => removeMaterial(index)}>
                  Удалить
                </button>
              </td>
            </tr>
          ...
          {services.map((item, index) => (
            <tr key={`service-${index}`}>
              ...
              <td>
                <button className="btn btn-danger btn-sm" onClick={() => removeService(index)}>
                  Удалить
                </button>
              </td>
            </tr>
      ...
      <Button variant="contained" onClick={() => alert("Генерируем бро!!!")} className="mt-3">Сгенерировать  PDF</Button>
    </div>
  );
};
